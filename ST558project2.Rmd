...
# `r toupper(paste(params$weekday,"ANALYSIS OUTput"))`
...
## 1. Read in datasets
```{r read_data, message=F, cache=TRUE}
...
```
The subset of `r params$weekday` data has `r nrow(fitData_log)` records and `r ncol(fitData_log)` features. Random sample of 70% of the data (`r nrow(training)` rows) as the training dataset. The other 30% of the data (`r nrow(testing)` rows) as the testing dataset.
...
For linear regression, I first fit a **full model** with all predictor features included, then a backward step-wise selection method (step function) is going to be used to select important features in the **reduced model** using **AIC** as the major selection criterion. Also, **adjusted R square, AICc, BIC** from two models are also listed.
...
The reduced model have `r summary(redModel)$df[1]-1` predictors which include features: `r attr(summary(redModel)$terms,"term.labels")` .
...
lmPredAccRate
```
For the reduced linear regression model, the prediction accurate rate is `r lmPredAccRate` with `r params$weekday` data. 
...
```
The final model selected from the random forest methods has `r rfModel$finalModel["mtry"]` mTry which is the number of variables available for splitting at each tree node. 
...
For the random forest model, the prediction accurate rate is `r rfPredAccRate` with `r params$weekday` data. 
...
```
For `r params$weekday` data, the `r ifelse(lmPredAccRate>rfPredAccRate, "linear model", "random forest")` model do a better job in the term of prediction accuracy.
